...
\begin{tabular}{@{}rl@{}}
  ...
  5 \texttimes & Trim potentiometer with center detent\footnote{Potentiometers with a center detent snap when they are turned through their center position} \\
  9 \texttimes & Trim potentiometer without a detent \\
  1 \texttimes & D-shaped potentiometer with center detent\footnote{See footnote 1} \\
  4 \texttimes & D-shaped potentiometer without a detent \\
  ...
  1 \texttimes & M2 slotted screw, M2 washer\footnote{Only available on newer kits, starting with serial number 220}, M2 Phillips screw, \\
               & standoff \\
\end{tabular}
...
Start by soldering the connectors. These include the power and I2C connectors,
and the connectors through which the Daisy Patch Submodule attaches to the main
PCB. For each connector, follow the same procedure:

\begin{enumerate}
  \item Put the connector through its footprint. Make sure to put it on the
    correct side of the PCB, where the connector is marked.
...
Solder the following connectors this way:

\begin{itemize}
  \item The male 2\texttimes5 connector, in the footprint marked with a white
    stripe and a label ``RED~-12V''. Put the side with shorter legs through the
    board.
  \item The male 1\texttimes3 connector, in the ``I2C'' footprint.
  \item The four female 2\texttimes5 connectors, in the footprints marked
    ``SM''.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Place the 5 D-shaped potentiometers in ``Pot''
    footprints\footnote{On older PCB versions, the ``Pot'' footprints were labeled as ``Green pot''.}. If the big legs are hard to snap in, flatten them with pliers first. Do not solder the legs yet. Use the appropriate potentiometer for the bottom-most place marked as ``Pot w/ center detent''.
  \item Similarly, place the 14 trimmer potentiometers in the ``Trimmer''
    footprints\footnote{On older PCB versions, the ``Trimmer'' footprints were labeled as ``Blue pot''.}, some of which are marked for center detent.
  ...
  \item Secure the front side of the standoff with the slotted screw and its
    washer\footnote{Only available on newer kits, starting with serial number 220}. Tighten all the potentiometers and 3.5mm jack sockets in place with
...
\begin{enumerate}
  \item Solder in all the rotary potentiometers and jacks.
...
The module is now complete. Have fun! You can find the user manual on \url{https://zlosynth.com/kaseta-user-manual.pdf}.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.9\linewidth]{p05.jpg}
  \caption{\centering Top, left, bottom and right side view}
\end{figure}

\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\linewidth]{p06.jpg}
