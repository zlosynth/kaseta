...
There is one audio input IN and stereo outputs LEFT and RIGHT.
...
The four CONTROL inputs accept CV from \textminus5 to +5 V and can be mapped to
...
Calibration, mapping and all secondary parameters are persisted between restarts
...
The PRE-AMP attribute attenuates or amplifies the signal received via INPUT between silence and +28 dB. If the input signal is boosted too hard and starts clipping, the display will start blinking.

\section{Internal oscillator}

The INPUT signal can be replaced with an internal oscillator. This oscillator consists of two sine waves, one of which is a slightly detuned sub-octave.

To enable this feature, turn the PRE-AMP knob to the max while holding the button. The PRE-AMP knob controls the pitch. If a control input is mapped to the pitch, it will follow the 1V/oct standard.
